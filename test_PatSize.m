...
mask = 0;
cImg = imread('images/hotel-3.jpg');
...
for i = 1:19
    patPara.patSize = 5 + 2*i;
    patPara.kNN = min(floor(patPara.patSize^2/2), 50);
    patPara.epsilon = min(patPara.patSize/2, 4);
    
    [ rImg ] = localColorization( gImg, propD, 0.5, patPara);
    PSNR(i) = psnr(rImg, cImg);
    SSIM(i) = ssim(rImg, cImg);
    recover{i} = rImg;
    
    save('hotelPatSize.mat');    
